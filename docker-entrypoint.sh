...
# patching config
sed -i "s#\"remote\":.*#\"remote\": \"${SELENIUM_URL}\",#g" test/config.json
sed -i "s#\"server\.hostname\":.*#\"server\.hostname\": \"${IPM_HOST}\",#g" test/config.json
sed -i "s#\"ipm\.port\":.*#\"ipm\.port\": \"${IPM_PORT}\",#g" test/config.json
sed -i "s#\"dcm\.url\":.*#\"dcm\.url\": \"${DCM_URL}\",#g" test/config.json
